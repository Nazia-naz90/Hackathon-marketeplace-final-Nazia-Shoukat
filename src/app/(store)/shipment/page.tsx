...
const ShippingRatesPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post("/shipengine/get-rates", {
        shipeToAddress,
      ...
      if (axios.isAxiosError(error)) {
        setErrors([error.response?.data?.message || "An error occurred while fetching rates."]);
      } else {
  ...
  const handleCreateLabel = async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        setErrors([error.response?.data?.message || "An error occurred while creating the label."]);
      } else {
        setErrors(["An unexpected error occurred."]);
      }
    } finally {
  ...
  return (
    <div className="min-h-screen text-black bg-gray-100 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-6">
              ...
              {rates.map((rate) => (
                <div
                  ...
                  className={`p-4 border rounded-lg shadow-md transition-transform transform hover:scale-105 cursor-pointer ${
                    rateId === rate.rateId ? "border-blue-500 bg-blue-100" : "border-gray-200 bg-gray-50"
                  }`}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-gray-800 font-semibold">
                        <strong>Cost:</strong> {rate.shippingAmount.amount} {rate.shippingAmount.currency}
                      </p>
                    </div>
                  </div>
                </div>
        ...
        {labelPdf && (
          <Link href={labelPdf} passHref>
            <a target="_blank" className="px-4 py-2 bg-green-500 text-white rounded-md hover:bg-green-600">
              Download Label
            </a>
          </Link>
        )}
